...
TrajectoryGenerator::TrajectoryGenerator(
  ...
  nh_private_.param("planning/target_frame", target_frame_, string("odom"));
  nh_private_.param("planning/arrive_thresh", arrive_thresh_, 0.5);
  ...
  odom_sub_ =
      nh_.subscribe("odometry/filtered", 1, &TrajectoryGenerator::odomCallback, this);
  ...
  vis_local_min_max_pub_ = nh_private_.advertise<visualization_msgs::Marker>("local_min_max", 1);
}
void TrajectoryGenerator::execTimerCallback(const ros::TimerEvent& e) {
  ...
  switch (robot_state_) {
    case INIT: {
      if (!has_odom_)
        return;
      // cout << "udpate map boundary." << endl;
      // safe_corridor_.updateMapBoundary(odom_pos_);
      safe_corridor_.visMapBoundary();
...
void TrajectoryGenerator::odomCallback(const nav_msgs::OdometryConstPtr& odom) {
  ...
  // ROS_INFO("Odom: x: %f, y: %f, yaw: %f", current_x, current_y, yaw);
  Vector2i grid = safe_corridor_.getGridmapIndex(Vector2d(current_x, current_y));
  // ROS_INFO("Odom: grid_x: %d, grid_y: %d", grid(0), grid(1));
}

void TrajectoryGenerator::visLocalMinMax(const Eigen::Vector2d& local_min, const Eigen::Vector2d& local_max) {
  visualization_msgs::Marker marker;
  
  marker.header.frame_id = "odom";  // Change to "odom" if needed
  marker.header.stamp = ros::Time::now();
  marker.ns = "local_min_max";
  marker.id = 0;
  ...
  p1.x = local_min(0);  p1.y = local_min(1);
  p2.x = local_max(0);  p2.y = local_min(1);
  p3.x = local_max(0);  p3.y = local_max(1);
  p4.x = local_min(0);  p4.y = local_max(1);
  ...
  vis_local_min_max_pub_.publish(marker);
}
...
void TrajectoryGenerator::laserToMapCallback(
  ...
  for(int i = 0; i < (int)local_pointcloud.points.size(); i++) {
    if(local_pointcloud.points[i].x <= 0.01 && local_pointcloud.points[i].y <= 0.01)
      continue;
  ...
  Eigen::Vector2d local_max(odom_pos_(0) + update_range_ / 2.0,
                            odom_pos_(1) + update_range_ / 2.0);
  visLocalMinMax(local_min,local_max);
  safe_corridor_.updateLocalMap(local_min, local_max);
  // ROS_WARN("updated local map");
...
void TrajectoryGenerator::pub_traj2ctrl(
  ...
  traj_msg.header.frame_id = std::string("odom");
  // traj_msg.header.frame_id = std::string("camera_init");
